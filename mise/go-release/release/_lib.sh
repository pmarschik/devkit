...
#
# These tasks release a Go workspace repo: one commit carries the root tag
# vX.Y.Z and a <subdir>/vX.Y.Z tag for every other module in go.work. A module
# reaches its users through the Go module proxy, so the release is finished
# when the tags are on GitHub and the proxy serves them. GoReleaser, if the
# repo runs it, only adds binaries on top of that.
#
...
#                           never wins the "latest tag" race
#   CHANGELOG.md            written by git cliff, formatted by hk
...
# Re-runnable: tags of the same name move onto ${2}. That matters in jj, where
# reviewing a prepared release and amending it produces a new commit id while
# the tags stay behind on the old one.
